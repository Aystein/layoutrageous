import { createContentNode, createParentNode } from "./util";
...
/**
 * Note: All actions directly mutate state and are designed to be used with immer!
 */
export function createTactileAdapter<Data>({
    sortComparer,
}: {
    sortComparer?: (a: DivideNode<Data>, b: DivideNode<Data>) => number;
}) {
    const sortIds = (state: TreeState<Data>) => {
        if (sortComparer) {
            state.ids.sort((a, b) => sortComparer(state.nodes[a], state.nodes[b]));
        }
    };

    const deleteNode = (state: TreeState<Data>, id: string) => {
    ...
    const getInitialState = (): TreeState<Data> => ({
        ids: [],
        nodes: {},
        root: undefined,
    });

    const updateGrowthValues = (state: TreeState<Data>, values: Record<string, number>) => {
    ...
    const visit = (state: TreeState<Data>, visitor: (parent: DivideParentNode | undefined, node: DivideNode<Data>) => void) => {
        if (!state.root) {
            return;
        }

        const recursiveVisit = (parent: DivideParentNode | undefined, node: DivideNode<Data>) => {
            visitor(parent, node);
    
            if (node.type === 'parent') {
                node.children.forEach((child) => recursiveVisit(node, state.nodes[child]));
            }
        }
    ...
    const applyInsert = (state: TreeState<Data>, toMove: string, target: string, direction: 'left' | 'right' | 'top' | 'bottom') => {
        const toMoveNode = state.nodes[toMove];
        const targetNode = state.nodes[target];

        // Remove node that we want to move
        deleteNode(state, toMove);
        ...
        if (targetNode.type === 'content') {
            ...
            if (targetNode.direction === 'row' && (direction === 'left' || direction === 'right')) {
                if (direction === 'left') {
            ...
            } else if (targetNode.direction === 'column' && (direction === 'top' || direction === 'bottom')) {
                if (direction === 'top') {
                    targetNode.children = [toMove, ...targetNode.children];
                }
                if (direction === 'bottom') {
                ...
                const replacementNode = createParentNode({
                    direction: direction === 'left' || direction === 'right' ? 'row' : 'column',
                    children: direction === 'left' || direction === 'top' ? [toMove, target] : [target, toMove]
                }, state);
     ...
     * as the container
     */
    const repairDegenerateTree = (state: TreeState<Data>) => {
        if (!state.root) {
            return;
        }
        ...
        const recursiveRepair = (node: DivideNode<Data>) => {
            ...
            if (node.type === 'parent' && node.children.length > 1) {
                ...
                if (children.find((child) => child.type === 'parent' && (child as DivideParentNode).direction === direction)) {
                    // We have a child that is a parent and has the same direction
                    const newChildren = children.map((child) => {
                        if (child.type === 'parent' && (child as DivideParentNode).direction === direction) {
                            // Remove node
    ...
    const greedySplit = (state: TreeState<Data>, data: Data) => {
        if (!state.root) {
            ...
            visit(state, (parent, node) => {
                if (node.type === 'content') {
                    const boundingRect = document.getElementById(node.id)!.getBoundingClientRect();
                    if (boundingRect.width * boundingRect.height > max) {
                        max = boundingRect.width * boundingRect.height;
                        largestNode = node;
                        direction = boundingRect.width > boundingRect.height ? 'row' : 'column';
                    }
                }
            });
